...
def add_portchannel_member(ctx, portchannel_name, port_name):
    ...
    #Dont allow adding memberports which have ip adressess configured in it
    for k,v in db.get_table('INTERFACE').iteritems():
        ...
        if k == port_name:
            ...
            return
    #Dont allow adding memberports which have vlan configured under it
    for k,v in db.get_table('VLAN_MEMBER'):
        if v == port_name:
...
def del_vlan(ctx, vid):
    db = ctx.obj['db']
    #New Logic. If IP is configured or have ports/POs configured as member dont allow removal of vlan
    for k,v in db.get_table('VLAN_INTERFACE').iteritems():
        if k == 'Vlan{}'.format(vid):
            ctx.fail("%s is configured with IP %s, remove the IP configuration "%(str(k),str(v)))
    for k, v in db.get_table('VLAN_MEMBER'):
        if k == 'Vlan{}'.format(vid):
            ctx.fail("%s  is member of %s. Remove vlan membership before removing vlan "%(str(v), str(k)))
    # Old Logic. Autoremove members if vlan is deleted.
...
def add_vlan_member(ctx, vid, interface_name, untagged):
    ...
    # Validate if interface is already a member of port-channel
    for k,v in db.get_table('PORTCHANNEL_MEMBER'):
        if v == interface_name:
            ctx.fail(" {} is member of a port-channel".format(interface_name))
            return
